...
export default class HolidayRepository {
  ...
  async isHoliday(date, location) {
    try {
      ...
      const holiday = await Holiday.findOne({
        ...
        location: location, // This will match if the location exists in the array
      });
  ...
  async getNextHolidays(currentDate) {
    try {
      return await Holiday.find({ start_date: { $gte: currentDate } }).sort({
        start_date: 1,
      });
    } catch (error) {
