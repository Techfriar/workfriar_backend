...
import { AddProjectStatusReportRequest } from "../../requests/admin/add-project-status-report-request.js";
...
export default class ProjectStatusReportController {
  ...
  async addReport(req, res) {
    try {
      const validatedData = await new AddProjectStatusReportRequest(
        req
      ).validate();
      ...
      if (reportDetails) {
        const reportData =
          await ProjectStatusReportResponse.formatGetAllReportResponse(
            reportDetails
          );
  ...
  async getAllReports(req, res) {
    try {
      ...
      const formattedReports = await Promise.all(
        reports.map(
          async (report) =>
            await ProjectStatusReportResponse.formatGetAllReportResponse(report)
        )
      );
  ...
  async getReportById(req, res) {
    try {
      ...
      const reportData =
        await ProjectStatusReportResponse.formatGetByIdReportResponse(report);
      return res.status(200).json({
        status: true,
  ...
  async updateReport(req, res) {
    try {
      const validatedData = await new UpdateProjectStatusReportRequest(
        req
      ).validate();
      ...
      if (reportDetails) {
        const reportData =
          await ProjectStatusReportResponse.formatGetByIdReportResponse(
            reportDetails
          );
        return res.status(200).json({
          status: true,
   ...
   *
   * @swagger
   * /project-status-report/dropdown/{type}:
   *   post:
   *     tags:
   *       - Project Status Report
   *     summary: Get dropdown data for projects or project leads
   *     parameters:
   *       - in: path
   *         name: type
   *         required: true
  ...
  async getDropdownData(req, res) {
    ...
    if (!["projects", "leads"].includes(type)) {
      return res.status(400).json({
        status: false,
        message: "Invalid dropdown type. Must be 'projects' or 'leads'.",
      });
    }

    try {
      const data = await reportRepo.getDropdownData(type);
      const entityType =
        type === "projects" ? "Project names" : "Project leads";
